  ...
  try
  {
    ...
    if (!$teamName == null && !$teamManager==null) // Prepare statement only if there is valid input on both fields
    {
      $query = 'UPDATE team SET
                    TeamName = :teamName, TeamManager =  :teamManager
                  WHERE
                    TeamID = :teamID';
    ...
    else if (!$teamManager==null) //Prepares statement only if there is a valid input on the DriverName field
    {
      $query = 'UPDATE team SET
...
<html lang="en">
  ...
  <body>
    ...
    <?php // this will confirm to the user if a value was updated or not.
     if ($success) {
      echo "<p>Below is the information updated TeamID: $teamID.</p>";
    } else {
